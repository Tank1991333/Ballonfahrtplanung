...
/*
 * Low levels use ICON-D2 fixed heights. From 200 m upward, GFS pressure
 * levels are used. Pressure levels are not exact AGL heights; they are the
 * closest available atmospheric model surfaces and are labelled accordingly.
 */
const definitions = [
  [10, "icon_d2", "ICON-D2"],
  [80, "icon_d2", "ICON-D2"],
  [120, "icon_d2", "ICON-D2"],
  [180, "icon_d2", "ICON-D2"],
  [200, "gfs_seamless", "GFS (1000 hPa)", "1000hPa"],
...
function valueAt(data, variable, timestamp) { const times = (data.hourly?.time || []).map((time) => Date.parse(`${time}Z`)), values = data.hourly?.[variable] || []; if (!times.length || !values.length) throw new Error(`Keine Daten für ${variable} erhalten.`); let i = times.findIndex((time) => time >= timestamp); if (i < 0) i = times.length - 1; if (!i) return Number(values[0]) || 0; const ratio = (timestamp - times[i - 1]) / (times[i] - times[i - 1]); return (Number(values[i - 1]) || 0) + ((Number(values[i]) || 0) - (Number(values[i - 1]) || 0)) * ratio; }
...
async function forecasts(lat, lon, selected) { const groups = new Map(); selected.forEach((height) => { const definition = byHeight.get(height); if (!groups.has(definition.model)) groups.set(definition.model, []); groups.get(definition.model).push(variable(definition, "wind_speed"), variable(definition, "wind_direction")); }); const forecastData = new Map(); await Promise.all([...groups].map(async ([model, variables]) => forecastData.set(model, await load(model, lat, lon, variables)))); return forecastData; }
function build(definition, forecast, lat, lon, start, hours) { let position = [lat, lon]; const points = [position]; const seconds = 900; const steps = Math.ceil(hours * 3600 / seconds); for (let step = 1; step <= steps; step += 1) { const timestamp = start + step * seconds * 1000; position = move(position, valueAt(forecast, variable(definition, "wind_speed"), timestamp), valueAt(forecast, variable(definition, "wind_direction"), timestamp), seconds); points.push(position); } return points; }

async function runSimulation() {
  const lat = Number(latInput.value), lon = Number(lonInput.value), hours = Number(duration.value), selected = [...levels.querySelectorAll("input:checked")].map((input) => Number(input.value)), currentRequest = ++requestId;
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return setStatus("Die Startkoordinaten sind ungültig.", "error");
  ...
  if (!Number.isFinite(start)) return setStatus("Datum oder Startzeit ist ungültig.", "error");
  clearLayers(); runButton.disabled = true; runButton.textContent = "Wetterdaten werden geladen..."; setStatus("ICON-D2- und GFS-Winddaten werden geladen...", "loading");
  try {
    const data = await forecasts(lat, lon, selected); if (currentRequest !== requestId) return;
    const all = []; let rows = "";
    selected.forEach((height, index) => {
      const definition = byHeight.get(height), forecast = data.get(definition.model), points = build(definition, forecast, lat, lon, start, hours), end = points.at(-1), endTime = start + (points.length - 1) * 900000, speed = valueAt(forecast, variable(definition, "wind_speed"), endTime), direction = valueAt(forecast, variable(definition, "wind_direction"), endTime), color = colors[index % colors.length];
      const line = L.polyline(points, { color, weight: 4, opacity: .82, bubblingMouseEvents: false }).addTo(map).bindPopup(`<b>Windroute</b><br>${formatAltitude(height)}<br>${definition.sourceLabel}`);
      const endMarker = L.circleMarker(end, { radius: 7, fillColor: color, color: "#fff", weight: 2, fillOpacity: 1, bubblingMouseEvents: false }).addTo(map);
      layers.push(line, endMarker); all.push(...points);
    ...
    const bounds = L.latLngBounds(all); if (bounds.isValid()) map.fitBounds(bounds.pad(.12), { maxZoom: 13 });
    setStatus(`${selected.length} Trajektorien mit echten Modellvorhersagen berechnet.`, "success");
  } catch (error) { if (currentRequest === requestId) { clearLayers(); results.innerHTML = emptyResults; setStatus(error.message || "Berechnung fehlgeschlagen.", "error"); } }
  finally { if (currentRequest === requestId) { runButton.disabled = false; runButton.textContent = "Simulation ausführen"; } }
}
